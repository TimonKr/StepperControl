...
def parse_command(line, motor):
    COMMANDS = {"SET": {"MOVE": bool,  "RES": int,"DIR": bool,  "STEPS": int, "SPEED": float, "LOWERLIM": int, "UPPPERLIM": int , "PULPIN": int, "DIRPIN": int}, 
                "GET": {"MOVE": bool, "RES": int, "DIR": bool, "STEPS": int, "SPEED": float, "LOWERLIM": int, "UPPPERLIM": int}}
    ...
    if cmd == "SET":
        if len(parts) < 3:
            print("Invalid SET command. Usage: SET <RESOLUTION|SPEED> <value>")
            return
        subcmd = parts[1]
        ...
        if len(parts) < 2:
            print("Invalid START command. Usage: e.g. START STEPS CW 200")
            return
        subcmd = parts[1]
